...
FROM dbex.pessoa pe, dbex.paciente pa
WHERE pe.cpf = pa.cpf_pessoa 
...
FROM dbex.pessoa pe, dbex.paciente pa
WHERE pe.cpf = pa.cpf_pessoa 
  AND (pe.telefone IS NULL OR pe.telefone = '');
...
FROM dbex.pessoa pe, dbex.paciente pa, dbex.agendamento a
WHERE pe.cpf = pa.cpf_pessoa
  AND pa.cpf_pessoa = a.cpf_paciente
  AND EXTRACT(DAY FROM pe.data_nasc) = EXTRACT(DAY FROM a.dh_consulta)
...
FROM dbex.pessoa pe, dbex.medico m, dbex.medicoespecialidade me, dbex.especialidade esp
WHERE pe.cpf = m.cpf_pessoa
  AND m.cpf_pessoa = me.cpf_medico
  AND me.id_especialidade = esp.id;
...
SELECT pe.nome, m.crm, 
       (SELECT COUNT(*) 
        FROM dbex.agendamento a 
        WHERE a.cpf_medico = m.cpf_pessoa) AS quantidade_consultas
FROM dbex.medico m, dbex.pessoa pe
WHERE m.cpf_pessoa = pe.cpf;
